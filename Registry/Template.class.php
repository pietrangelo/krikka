 ...
 * Include our page class, and build a page object to manage the content and
...
class Template {
     ...
     * @param Strting $tag the tag where we insert the template e.g. {hello}
    ...
    private function replaceTags($pp = false) {
        ...
        {
            //if the tag is an array, then we need to do more than a simple find
            // and replace!
            if(is_array($data))
            {
                if($data[0] == 'SQL')
    ...
    private function replaceDBTags($tag, $cacheId) {
            ...
            if(in_array($tag, $apdKeys))
            {
                ...
                foreach($tag as $ntag => $data)
                {
                    ...
                    if(array_key_exists($ntag, $apd[$tag]))
                    {
                        //YES it is
                        $extra = $apd[$tag][$ntag];
                        //does the tag equel the condition?
                        if($data == $extra['condition'])
                        {
                            //Yes! Replace the extratag with the data
        ...
        $newContent = str_replace('<!-- STAR ' . $tag . ' -->' . $blockOld . '<!-- END ' . $tag . ' -->', $block, $pageContent);
